...
const UpdateRecentEventModal = ({
  id,
  img,
  isOpen,
  modalOpenController,
}: TUpdateRecentEventModal) => {
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: async (data: TData) => {
      return await axios.patch(`/recent-event/${id}`, data as TData);
    },
    onSuccess: () => {
      setIsSuccess(true);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Update Recent Events</DialogTitle>
        </DialogHeader>
        <>
          {isError ? (
            <div>
              ...
              {isSuccess ? (
                <div>
                  <h2 className=" text-center text-green-600">
                    Suceessfully Update Recent Event!
                  </h2>
                </div>
              ) : (
                <>
...
export default UpdateRecentEventModal;
